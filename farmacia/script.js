...
function pesquisarProdutos() {
    ...
    try {
        if (db) {
            // Busca usando SQL (prepared statement) para evitar carregar/filtrar tudo no JS
            const like = `%${termo}%`;
            const stmt = db.prepare(`
                SELECT id, nome, tipo, categoria, tarja,
                       descricao, quantidade, preco, promocao,
                       precoAntigo, desconto, imagem
                FROM produtos
                WHERE lower(nome) LIKE ? OR lower(descricao) LIKE ? OR lower(tipo) LIKE ?
            `);
            stmt.bind([like, like, like]);

            while (stmt.step()) {
                const linha = [
                    stmt.get(0), stmt.get(1), stmt.get(2), stmt.get(3),
                    stmt.get(4), stmt.get(5), stmt.get(6), stmt.get(7),
                    stmt.get(8), stmt.get(9), stmt.get(10), stmt.get(11)
                ];
                resultados.push(linhaProdutoParaObjeto(linha));
            }
            stmt.finalize();
        } else {
            // Fallback para quando o SQLite não estiver disponível
        ...
        // Em caso de erro, usar fallback em memória
        resultados = produtos.filter(p =>
            p.nome.toLowerCase().includes(termo) ||
            p.descricao.toLowerCase().includes(termo) ||
            p.tipo.toLowerCase().includes(termo)
